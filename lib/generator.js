...
function generatePictureMarkup(id, data) {
	...
	if (isAboveTheFold) {
		...
		const mobileMeasurements = measurements["320x800"];
	...
	const breakpointMap = {
		"320x800": { name: "Mobile", mediaQuery: "(max-width: 600px)" },
	...
	const orderedBreakpoints = ["320x800", "1024x800", "1920x1080"];
	...
	markup += `\t<img src="{% getUrl "${imgSrc}" | resize({ width: ${scaledWidth}, height: ${scaledHeight} }) | jpeg %}" alt="${imgAlt}" width="${fallbackWidth}" height="${fallbackHeight}"${
