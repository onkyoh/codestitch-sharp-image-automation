...
async function measureImagesOnPage(url, pageName, outputDir, skinny = false, skinnyFormat = null) {
	...
	try {
		...
		for (let i = 0; i < allPictureElements.length; i++) {
			...
			const imgInfo = await imgElement.evaluate((img) => {
				return {
					...
					alt: img.alt || "",
					id: img.id || "",
					isPNG: img.src.toLowerCase().endsWith(".png"),
				};
			});
		...
		if (Object.keys(results).length > 0) {
			...
			for (const [id, data] of Object.entries(results)) {
				const markup = generator.generatePictureMarkup(id, data, skinny, skinnyFormat);
				allMarkup += markup + "\n\n";
			}
...
async function scanProject(options) {
	try {
		...
		for (let i = 0; i < pageList.length; i++) {
			...
			const result = await measureImagesOnPage(page.url, page.pageName, options.outputDir, options.skinny, options.skinnyFormat);
			results.push(result);
		}
